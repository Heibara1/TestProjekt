...
public class DataService
{
    ...
    public DagligFast OpretDagligFast(int patientId, int laegemiddelId, double antalMorgen, double antalMiddag, double antalAften, double antalNat, DateTime startDato, DateTime slutDato)
    {
        ...
        var dagligFastOrdination = new DagligFast(startDato, slutDato, laegemiddel,
    ...
    public DagligSkæv OpretDagligSkaev(int patientId, int laegemiddelId, Dosis[] doser, DateTime startDato, DateTime slutDato)
    {
        ...
        var dagligSkævOrdination = new DagligSkæv(startDato, slutDato, laegemiddel);
        dagligSkævOrdination.doser = doser.ToList(); 
    ...
    public string AnvendOrdination(int id, Dato dato)
    {
        ...
        return "Dosis til den specifikke dato";
    }
    ...
    public double GetAnbefaletDosisPerDøgn(int patientId, int laegemiddelId)
    {
        ...
        if (patient.vaegt <= 0)
        {
            throw new ArgumentException("Vægten kan ikke være nul eller negativ.");
        }

        if (patient.vaegt < 25)
        {
            dosis = laegemiddel.enhedPrKgPrDoegnLet * patient.vaegt;
        }
        else if (patient.vaegt >= 25 && patient.vaegt <= 120)
        {
            dosis = laegemiddel.enhedPrKgPrDoegnNormal * patient.vaegt;
        }
        else
        {
            dosis = laegemiddel.enhedPrKgPrDoegnTung * patient.vaegt;
        }
